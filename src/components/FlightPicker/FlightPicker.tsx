...
const FlightPicker = ({
  ...
  const parseDate = (dateStr: string) => {
    if (/^\d{2}-\d{2}-\d{4} \d{1,2}:\d{2} [AP]M$/.test(dateStr)) {
      return parse(dateStr, "dd-MM-yyyy hh:mm a", new Date());
    }

    if (/^\d{4}-\d{1,2}-\d{1,2} \d{1,2}:\d{2}$/.test(dateStr)) {
      return parse(dateStr, "yyyy-M-d HH:mm", new Date());
    }
    ...
    if (/^\d{2}-\d{2}-\d{4}\d{1,2}:\d{2} [AP]M$/.test(dateStr)) {
      ...
      return parse(fixed, "dd-MM-yyyy hh:mm a", new Date());
    }
                ...
                >
                  ...
                  /> */}
                                ...
                                <div className="text-lg font-medium text-gray-900 col-start-3 row-start-1">
                                  {formatTime(segment.departureAirport.time)} -{" "}
                                ...
                                {flight?.layovers?.[index] && (
                                  <div className="col-start-3 col-span-2">
                                    <div className="space-y-1 my-2 py-3 border-t-2 border-b-2 border-gray-200">
                                      <div>
                                        {flight.layovers[index].durationLabel}{" "}
                                        layover - {flight.layovers[index].city}{" "}
